...
if (gallery) {
      ...
      lightbox.pswp.ui.registerElement(
        {
          ...
          html: 'Details',
          onClick: (event, el) => {
            console.log(pswp.currSlide.data.element.children[2].content);
            window.location.replace(pswp.currSlide.data.element.children[2].content);
          }
        },
        {
  ...
  lightbox.on("change", () => {
    console.log("lightbox.on.change")
    const target = lightbox.z
    // lightbox.pswp.currSlide?.data?.element?.children[2].content)
    history.replaceState("", document.title, "#" + target);
  });

  lightbox.on("close", () => {
    console.log("lightbox.on.close")
  ...
  if (window.location.hash.substring(1).length > 1) {
    ...
    for (let i = 0; i < items.length; i++) {
      if (items[i].dataset["pswpTarget"] === target) {
        console.log("calling lightbox.loadAndOpen")
